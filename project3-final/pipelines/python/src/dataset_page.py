...
def make_ppi_interactive(dataset, X: np.ndarray, out_path: Path,
    ...
    # Color = mean expression, diverging blue-white-red centered at 0
    vmax_abs = float(max(abs(node_expr.min()), abs(node_expr.max()), 1e-9))
    expr_norm = mcolors.TwoSlopeNorm(vmin=-vmax_abs, vcenter=0.0, vmax=vmax_abs)
...
<p>
  ...
  Node color = average expression across the cohort (blue = negative, white ≈ 0, red = positive);
