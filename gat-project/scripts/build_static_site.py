...
PROJECT_TITLE_HTML = PROJECT_TITLE["en"]
...
if len(G_full) == 0:
    ...
    expr_vals_full = [float(mean_expr_all[n]) for n in nodes_full]
    expr_absmax = max((abs(v) for v in expr_vals_full), default=1.0) or 1.0
...
<div class="explore-container">
  <div class="sidebar">
    ...
    <div class="control-group">
      <label>Min Confidence (<span id="conf-val">0</span>%)</label>
      <input type="range" id="conf-filter" min="0" max="100" step="5" value="0" oninput="document.getElementById('conf-val').innerText=this.value;">
    </div>
    ...
    <div style="margin-top: 1.5em;">
      ...
      <div class="control-group" style="display: flex; align-items: center; gap: 0.5em; margin-top: 1em;">
        <input type="checkbox" id="toggle-labels" onchange="renderGraph();" style="width: auto; cursor: pointer;">
        <label for="toggle-labels" style="margin-bottom: 0; cursor: pointer; text-transform: none; font-size: 0.85em; letter-spacing: normal;">Show Gene Names on Graph</label>
      </div>
    </div>
  </div>
  ...
  <div class="main-content">
    ...
    <div class="selected-pane" id="selected-info-pane" style="display: none;">
      ...
      <table id="selected-table">
        <thead>
          <tr id="selected-table-header">
            <!-- Headers filled dynamically in renderSelectedTableHeader() -->
          </tr>
        </thead>
        <tbody></tbody>
      </table>
...
let comparisonGroup = new Set([0, 1]); // Active in Mean / Addition
let minuendGroup = new Set([0]); // Subtraction Minuend list
let subtrahendGroup = new Set([1]); // Subtraction Subtrahend list
...
const predClsEl = document.getElementById("pred-cls");
...
function init() {{
  // Filter listeners
  [qEl, clsEl, predClsEl, splitEl, okEl, confFilterEl].forEach(el => el.addEventListener("input", filterSamples));
...
function toggleSample(idx) {{
  if (selectedSampleIds.has(idx)) {{
    ...
    comparisonGroup.delete(idx);
    minuendGroup.delete(idx);
    subtrahendGroup.delete(idx);
  }} else {{
    selectedSampleIds.add(idx);
    comparisonGroup.add(idx);
    
    // Sensible auto-distribution in Subtraction list
    if (minuendGroup.size === 0) {{
      minuendGroup.add(idx);
    }} else if (subtrahendGroup.size === 0) {{
      subtrahendGroup.add(idx);
    }}
  }}
  updateSelectedPane();
  renderGraph();
...
function selectAll(checked) {{
  ...
  EXPLORATION_DATA.samples.forEach(s => {{
    ...
    if (match) {{
      if (checked) {{
        selectedSampleIds.add(s.idx);
        comparisonGroup.add(s.idx);
      }} else {{
        selectedSampleIds.delete(s.idx);
        comparisonGroup.delete(s.idx);
        minuendGroup.delete(s.idx);
        subtrahendGroup.delete(s.idx);
      }}
    }}
  }});
...
function syncComparisonGroups() {{
  // Cleanup sets of any deselected records
  for (let id of comparisonGroup) {{
    if (!selectedSampleIds.has(id)) comparisonGroup.delete(id);
  }}
  for (let id of minuendGroup) {{
    if (!selectedSampleIds.has(id)) minuendGroup.delete(id);
  }}
  for (let id of subtrahendGroup) {{
    if (!selectedSampleIds.has(id)) subtrahendGroup.delete(id);
  }}
  
  // Auto-populate to ensure UI is never empty
  if (comparisonGroup.size === 0 && selectedSampleIds.size > 0) {{
    selectedSampleIds.forEach(id => comparisonGroup.add(id));
  }}
  if (minuendGroup.size === 0 && subtrahendGroup.size === 0 && selectedSampleIds.size > 0) {{
    const arr = Array.from(selectedSampleIds).sort((a,b) => a - b);
    if (arr.length > 0) minuendGroup.add(arr[0]);
    if (arr.length > 1) subtrahendGroup.add(arr[1]);
  }}
}}

function renderSelectedTableHeader() {{
  const tr = document.getElementById("selected-table-header");
  let compHeaders = "";
  if (currentAlgorithm === "subtraction") {{
    compHeaders = `
      <th style="text-align: center; width: 120px;">
        <div>Minuend</div>
        <div style="font-size:0.75em; font-weight:normal; margin-top:4px;">
          <a href="#" onclick="event.preventDefault(); batchToggleCompTable('min', true)">All</a> | 
          <a href="#" onclick="event.preventDefault(); batchToggleCompTable('min', false)">Clear</a>
        </div>
      </th>
      <th style="text-align: center; width: 120px;">
        <div>Subtrahend</div>
        <div style="font-size:0.75em; font-weight:normal; margin-top:4px;">
          <a href="#" onclick="event.preventDefault(); batchToggleCompTable('sub', true)">All</a> | 
          <a href="#" onclick="event.preventDefault(); batchToggleCompTable('sub', false)">Clear</a>
        </div>
      </th>
    `;
  }} else {{
    compHeaders = `
      <th style="text-align: center; width: 150px;">
        <div>Use in comparison</div>
        <div style="font-size:0.75em; font-weight:normal; margin-top:4px;">
          <a href="#" onclick="event.preventDefault(); batchToggleCompTable('comp', true)">All</a> | 
          <a href="#" onclick="event.preventDefault(); batchToggleCompTable('comp', false)">Clear</a>
        </div>
      </th>
    `;
  }}
  ...
  tr.innerHTML = `
    <th onclick="sortSelectedTable(0, 'int')">idx</th>
    <th onclick="sortSelectedTable(1, 'str')">patient id</th>
    <th onclick="sortSelectedTable(2, 'str')">true class</th>
    <th onclick="sortSelectedTable(3, 'str')">predicted</th>
    <th onclick="sortSelectedTable(4, 'str')">split</th>
    <th onclick="sortSelectedTable(5, 'float')">confidence</th>
    <th onclick="sortSelectedTable(6, 'str')">correct?</th>
    ${{compHeaders}}
  `;
}}

function batchToggleCompTable(type, checkAll) {{
  const tbody = document.querySelector("#selected-table tbody");
  if (!tbody) return;
  const rows = tbody.querySelectorAll("tr");

  // Get active filter parameters to make sure batch changes only affect visually matched rows
  const qv = qEl.value.toLowerCase();
  const clsv = clsEl.value;
  const predclsv = predClsEl.value;
  const splitv = splitEl.value;
  ...
  rows.forEach(row => {{
    const idx = parseInt(row.cells[0].innerText.trim());
    if (isNaN(idx)) return;

    const s = EXPLORATION_DATA.samples.find(sm => sm.idx === idx);
    if (!s) return;

    const baseFilterMatch = (!qv || s.id.toLowerCase().includes(qv)) &&
                            (!clsv || s.true === clsv) &&
                            (!predclsv || s.pred === predclsv) &&
                            (!okv || String(s.ok) === okv) &&
                            (splitv === "all" || s.split === splitv) &&
...
// If clearing, clear everything currently visible. If selecting all, only check rows passing the filters.
    if (!checkAll || baseFilterMatch) {{
      if (type === 'comp') {{
        if (checkAll) comparisonGroup.add(idx); else comparisonGroup.delete(idx);
      }} else if (type === 'min') {{
        if (checkAll) {{
          minuendGroup.add(idx);
          subtrahendGroup.delete(idx);
...
function handleCompToggle(idx, type, checked) {{
  if (type === 'comp') {{
    if (checked) {{
      comparisonGroup.add(idx);
    }} else {{
    ...
    if (checked) {{
      minuendGroup.add(idx);
      subtrahendGroup.delete(idx); // Mutually exclusive
    }} else {{
      minuendGroup.delete(idx);
    }}
  }} else if (type === 'sub') {{
    if (checked) {{
      subtrahendGroup.add(idx);
      minuendGroup.delete(idx); // Mutually exclusive
    }} else {{
      subtrahendGroup.delete(idx);
    }}
  }}
...
function updateSelectedPane() {{
  ...
  // Clean up any deleted objects from the sets, but do NOT remove items hidden by filters
  for (let id of comparisonGroup) {{ if (!selectedSampleIds.has(id)) comparisonGroup.delete(id); }}
  for (let id of minuendGroup) {{ if (!selectedSampleIds.has(id)) minuendGroup.delete(id); }}
  for (let id of subtrahendGroup) {{ if (!selectedSampleIds.has(id)) subtrahendGroup.delete(id); }}
  
  renderSelectedTableHeader();
  ...
  // Get active filter parameters to evaluate row visibility dynamically
  const qv = qEl.value.toLowerCase();
  const clsv = clsEl.value;
  const predclsv = predClsEl.value;
  const splitv = splitEl.value;
  ...
  tbody.innerHTML = activeIds.map(idx => {{
    ...
    // Evaluate if this specific row matches the visual filters
    const baseFilterMatch = (!qv || s.id.toLowerCase().includes(qv)) &&
                            (!clsv || s.true === clsv) &&
                            (!predclsv || s.pred === predclsv) &&
                            (!okv || String(s.ok) === okv) &&
    ...
    // Row remains visible if it passes the filters OR if it is actively checked for comparison
    let isCheckedInCurrentAlg = false;
    if (currentAlgorithm === "subtraction") {{
      isCheckedInCurrentAlg = minuendGroup.has(idx) || subtrahendGroup.has(idx);
    }} else {{
      isCheckedInCurrentAlg = comparisonGroup.has(idx);
    }}
    
    const rowStyle = (baseFilterMatch || isCheckedInCurrentAlg) ? "" : "display: none;";
    
    let compCols = "";
    if (currentAlgorithm === "subtraction") {{
      const isMin = minuendGroup.has(idx) ? "checked" : "";
      const isSub = subtrahendGroup.has(idx) ? "checked" : "";
      compCols = `
        <td style="text-align: center;">
          <input type="checkbox" ${{isMin}} onchange="handleCompToggle(${{idx}}, 'min', this.checked)">
        </td>
        <td style="text-align: center;">
          <input type="checkbox" ${{isSub}} onchange="handleCompToggle(${{idx}}, 'sub', this.checked)">
        </td>
      `;
    }} else {{
      const isComp = comparisonGroup.has(idx) ? "checked" : "";
      compCols = `
        <td style="text-align: center;">
          <input type="checkbox" ${{isComp}} onchange="handleCompToggle(${{idx}}, 'comp', this.checked)">
        </td>
      `;
    }}
...
function renderGraph() {{
  ...
  const compIds = Array.from(comparisonGroup);
  const numComp = compIds.length;
  
  if (numComp === 0 && currentAlgorithm !== "subtraction") {{
    Plotly.newPlot("plotly-graph", [], {{
      annotations: [{{ text: "Please check at least one 'Use in comparison' sample in the table below", showarrow: false, font: {{ size: 16 }} }}],
      xaxis: {{ visible: false }}, yaxis: {{ visible: false }}
    }});
    document.querySelector("#interactions-table tbody").innerHTML = "<tr><td colspan='5' style='text-align: center; color: #64748b;'>No samples active in comparison</td></tr>";
    return;
  }}
  ...
  // Calculate average gene expressions for active comparison nodes
  const exprIds = currentAlgorithm === "subtraction" ? Array.from(selectedSampleIds) : compIds;
  const numExpr = exprIds.length || 1;
  for (let i = 0; i < EXPLORATION_DATA.genes.length; i++) {{
    let sum = 0;
    for (let idx of exprIds) {{
      const s = EXPLORATION_DATA.samples.find(sm => sm.idx === idx);
      if (s) sum += s.expr[i];
    }}
    nodeExprs[i] = sum / numExpr;
  }}

  let subtitle = "";
  
  if (currentAlgorithm === "mean") {{
    subtitle = `Mean attention of ${{numComp}} sample(s)`;
    for (let i = 0; i < EXPLORATION_DATA.edges.length; i++) {{
      let sum = 0;
      for (let idx of compIds) {{
        const s = EXPLORATION_DATA.samples.find(sm => sm.idx === idx);
        if (s) sum += s.attn[i];
      }}
      edgeAttns[i] = sum / numComp;
    }}
  }} else if (currentAlgorithm === "addition") {{
    subtitle = `Addition attention of ${{numComp}} sample(s)`;
    for (let i = 0; i < EXPLORATION_DATA.edges.length; i++) {{
      let sum = 0;
      for (let idx of compIds) {{
        const s = EXPLORATION_DATA.samples.find(sm => sm.idx === idx);
    ...
    const minIds = Array.from(minuendGroup);
    const subIds = Array.from(subtrahendGroup);
    
    if (minIds.length === 0 || subIds.length === 0) {{
      Plotly.newPlot("plotly-graph", [], {{
        annotations: [{{ text: "Please select at least one Minuend and one Subtrahend sample in the table below", showarrow: false, font: {{ size: 16 }} }}],
        xaxis: {{ visible: false }}, yaxis: {{ visible: false }}
      }});
      document.querySelector("#interactions-table tbody").innerHTML = "<tr><td colspan='5' style='text-align: center; color: #64748b;'>Select Minuend and Subtrahend samples below</td></tr>";
      return;
    }}
    
    subtitle = `Subtraction: Mean of [${{minIds.join(", ")}}] − Mean of [${{subIds.join(", ")}}]`;
    for (let i = 0; i < EXPLORATION_DATA.edges.length; i++) {{
      let minSum = 0;
      for (let idx of minIds) {{
        const s = EXPLORATION_DATA.samples.find(sm => sm.idx === idx);
        if (s) minSum += s.attn[i];
      }}
      let minMean = minSum / minIds.length;
      
      let subSum = 0;
      for (let idx of subIds) {{
        const s = EXPLORATION_DATA.samples.find(sm => sm.idx === idx);
        if (s) subSum += s.attn[i];
      }}
      let subMean = subSum / subIds.length;
      
      edgeAttns[i] = minMean - subMean;
    }}
  }}
  ...
  for (let i = 0; i < EXPLORATION_DATA.edges.length; i++) {{
    ...
    let minuendMeanAttn = 0;
    let subtrahendMeanAttn = 0;
    if (currentAlgorithm === "subtraction") {{
      const minIds = Array.from(minuendGroup);
      const subIds = Array.from(subtrahendGroup);
      let minSum = 0;
      ...
      minuendMeanAttn = minIds.length ? minSum / minIds.length : 0;
      ...
      subtrahendMeanAttn = subIds.length ? subSum / subIds.length : 0;
    }}

    topEdgesList.push({{
      sName: EXPLORATION_DATA.genes[sNode],
      dName: EXPLORATION_DATA.genes[dNode],
      sAttn: minuendMeanAttn,
      dAttn: subtrahendMeanAttn,
      val: val,
      absVal: absVal
    }});
    ...
    if (currentAlgorithm === "subtraction") {{
      // Diverging colors: Red for positive (Minuend higher), Blue for negative (Subtrahend higher)
      if (val > 0) {{
  ...
  const nodeTrace = {{
    ...
    text: showLabels ? nodes.map(n => EXPLORATION_DATA.genes[n]) : null,
    textposition: 'top center',
    marker: {{
      size: 9,
      color: nodeColors,
  ...
  if (currentAlgorithm === "subtraction") {{
    ...
    colValAEl.innerText = "Mean Attn (Minuend)";
    colValBEl.innerText = "Mean Attn (Subtrahend)";
...
let selectedSortDirection = true; 
let currentSortCol = -1;

function sortSelectedTable(colIndex, type) {{
  const tbody = document.querySelector("#selected-table tbody");
  ...
  rows.sort((a, b) => {{
    ...
    if (type === 'int') {{
      return selectedSortDirection ? parseInt(valA) - parseInt(valB) : parseInt(valB) - parseInt(valA);
    }} else if (type === 'float') {{
      return selectedSortDirection ? parseFloat(valA) - parseFloat(valB) : parseFloat(valB) - parseFloat(valA);
    }} else {{
      return selectedSortDirection ? valA.localeCompare(valB) : valB.localeCompare(valA);
    }}
  }});
